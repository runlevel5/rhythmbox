...
class DarkLyricsParser(object):
    ...
    def search(self, callback, *data):
        ...
        self.artist_ascii = "".join(
            c for c in self.artist.lower() if c in string.ascii_letters
        )
        self.artist_ascii = self.artist_ascii.lower()
        if len(self.artist_ascii) == 0:
            print(
                "DarkLyricsParser: artist_ascii is empty for artist='%s', skipping"
                % self.artist
            )
            callback(None, *data)
            return
    ...
    def search_song(self, songlist, callback, *data):
        ...
        for line in matches:
            ...
            smvalue = stringmatch.string_match(
                title.lower().replace(" ", ""), self.title.lower().replace(" ", "")
            )
            if smvalue > min_song_match:
                best_match = self.SongFound(smvalue, title, number, album, artist)
                break
        if not best_match:
            callback(None, *data)
            return
