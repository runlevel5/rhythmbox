...
class LyricPane(object):
    def __init__(self, db, song_info):
        ...
        self.view.props.margin_start = 6
        self.view.props.margin_end = 6
        self.view.props.margin_top = 6
        self.view.props.margin_bottom = 6
...
class LyricWindow(Gtk.Window):
    def __init__(self, shell):
        ...
        # make the scrolled window expand to fill the window
        sw = lyrics_view.get_first_child()
        sw.set_vexpand(True)
        sw.set_hexpand(True)

        sp = shell.props.shell_player
        self.ppc_id = sp.connect(
            "playing-song-property-changed", self.playing_property_changed
        )
...
class LyricsDisplayPlugin(GObject.Object, RB.PeasActivatable):
    ...
    def do_activate(self):
        ...
        item = Gio.MenuItem.new(
            label=_("Song Lyrics"), detailed_action="win.view-lyrics"
        )
