...
class MagnatuneEntryType(RB.RhythmDBEntryType):
    ...
    def do_get_playback_uri(self, entry):
        ...
        if account_type != "none":
            uri = self.URIre.sub(
                "http://%s:%s@%s.magnatune.com/" % (username, password, account_type),
                uri,
            )
...
class MagnatuneConfig(GObject.GObject, RB.PeasGtkConfigurable):
    ...
    def do_create_configure_widget(self):
        ...
        # connect signals manually (builder.connect_signals removed in GTK4)
        for name in (
            "no_account_radio",
            "stream_account_radio",
            "download_account_radio",
        ...
        for entry_name in ("username_entry", "password_entry"):
            entry = builder.get_object(entry_name)
            focus_controller = Gtk.EventControllerFocus.new()
            focus_controller.connect(
                "leave",
                lambda ctrl, fn=account_details_changed: fn(ctrl.get_widget(), None),
            )
